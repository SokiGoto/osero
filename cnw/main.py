...
if new:
    # csvの読み込み
    df = pd.read_csv("../kihu/wthor_all.csv")
    ...
    np.save("../kihu/npy/x_train.npy", x_train)
    np.save("../kihu/npy/y_train_reshape.npy", y_train_reshape)
else:
    x_train = np.load("../kihu/npy/x_train.npy")
    y_train_reshape = np.load("../kihu/npy/y_train_reshape.npy")
...
callbacks = [#keras.callbacks.EarlyStopping(patience=0, verbose=1 ,mode='auto'), 
        keras.callbacks.ModelCheckpoint("saved_model_reversi/my_model", save_best_only=True)]
model.fit(x_train, y_train_reshape, epochs=600, batch_size=32, validation_split=0.2, callbacks=callbacks)
